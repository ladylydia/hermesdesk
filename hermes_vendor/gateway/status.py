...
def _write_runtime_status_inner(
    *,
    gateway_state: Any = _UNSET,
    exit_reason: Any = _UNSET,
    restart_requested: Any = _UNSET,
...
# ── Windows: replace file-based locks with kernel mutex ──────────────────
if _IS_WINDOWS:
    try:
        from .lock_win32 import (  # type: ignore[import-untyped]
            acquire_gateway_runtime_lock,
            release_gateway_runtime_lock,
            is_gateway_runtime_lock_active,
        )
    except ImportError as _e:  # pragma: no cover - embedded runtime
        import logging
        _log = logging.getLogger(__name__)
        _log.warning("lock_win32 unavailable, falling back to msvcrt.locking: %s", _e)
