//! Route C: short-lived bundled Python process for Feishu / Lark scan-to-create QR flow (`feishu_qr_worker.py`).
...
const PROGRESS_FILE: &str = "feishu_qr_progress.json";
...
/// Start `python.exe feishu_qr_worker.py` (CREATE_NO_WINDOW on Windows). Clears prior progress/result.
#[tauri::command]
pub async fn cmd_feishu_qr_start(
    app: AppHandle,
    ...
    let worker = bundle.join("feishu_qr_worker.py");
    if !worker.exists() {
        return Err(
            "feishu_qr_worker.py is missing from the runtime bundle. Rebuild the Python bundle."
                .into(),
        );
    }
    ...
    let mut slot = state.feishu_qr_child.lock().await;
    ...
    if let Some(out) = child.stdout.take() {
        tokio::spawn(log_pipe("feishu_qr.out", out));
    }
    if let Some(err) = child.stderr.take() {
        tokio::spawn(log_pipe("feishu_qr.err", err));
    }
...
#[tauri::command]
pub async fn cmd_feishu_qr_status(
    app: AppHandle,
    state: tauri::State<'_, crate::AppState>,
) -> Result<Value, String> {
...
#[tauri::command]
pub async fn cmd_feishu_qr_cancel(state: tauri::State<'_, crate::AppState>) -> Result<(), String> {
    let mut slot = state.feishu_qr_child.lock().await;
    if let Some(mut c) = slot.take() {
        let _ = c.start_kill();
    }
