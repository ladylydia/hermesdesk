...
| 属性 | 内容 |
|------|------|
| **Given** | 首次安装 HermesDesk，未配置过任何内容 |
| **When** | 按引导流程完成设置 |
| **Then** | 引导完成，进入 `/chat` 页面，可正常对话 |

**手工执行步骤（步骤级）：**
1. 首次启动 HermesDesk → 看到 Splash 页面 → 自动跳转到 Onboarding 引导页
2. **欢迎页**：看到 HermesDesk 介绍文案 → 点击"开始"
3. **选择提供商**：
   - 默认显示 DeepSeek（选中状态）
   - 点击"下一步"
4. **输入 API Key**：
...
| **Given** | 在 `/settings` 页面 |
| **When** | 切换字体大小选项（小 / 中 / 大） |
| **Then** | `/chat` 页面文本即时变化；重启 HermesDesk 后保持 |
...
- 代码块、附件预览等子组件也应跟随主题
- "跟随系统"选项：修改 Windows 主题后 HermesDesk 自动切换（可能需要重启生效）
...
5. 重启 HermesDesk → 回到 `/settings` → Telegram 区块仍显示"已配置"
...
5. 使用飞书手机 App 扫描二维码
6. 确认绑定
7. HermesDesk 中 QR 码弹窗关闭，飞书区块显示 `"已绑定 ✓"`
