...
HermesDesk **已产品化路线 C**：微信扫码走 **`weixin_qr_worker.py`** + `WeixinQrRouteCBlock`；同类「bundle 内 Python + Tauri IPC」模式亦用于 **QQ**（`qqbot_qr_worker.py`）、**飞书/Lark**（`feishu_qr_worker.py`）。Telegram 为 **token 表单**，不走扫码 worker。总览见 [gateway-desk-weixin-strategy.md](gateway-desk-weixin-strategy.md)。
...
2. **网络**：内嵌进程须能访问 `https://ilinkai.weixin.qq.com`（HermesDesk 的 `network_allowlist` overlay 需包含该主机；当前若未放行，要在 overlay 或配置里显式加入后再测）。
...
```powershell
Set-Location D:\project\hermesdesk
...
严格意义上的 **第二进程 + 空壳 Tauri** 可以不做：HermesDesk **已是** Tauri + 内嵌 Python。工程上最小验证集为：
