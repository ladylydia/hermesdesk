...
1. In **`strip_shims` / prune lists** for subsystems we do not load in the **web** interpreter (e.g. `tui_gateway/`, RL, some CLI-only modules), **or** loaded only in **separate** processes (e.g. real `gateway.run` child), or
...
| `hermes_cli/gateway.py`               | **CLI / optional**    | Interactive setup; desk uses Tauri + workers instead |
| `gateway/run.py`, `gateway/status.py` | **Shipped**             | Loaded by **separate** `gateway.run` child, not web `web_server` shim |
| `gateway/platforms/whatsapp.py`       | **In tree**           | Shipped with `hermes/`; adapters enabled per user `.env` |
...
- `strip_shims.py` — no-op **leaf** modules where `web_server` imports
  `gateway.status` / stubs `gateway.run.main` inside the **web** interpreter only;
  the real `gateway/` package remains importable for **`gateway.run`** subprocess
  and `gateway.session_context` (terminal approval).
